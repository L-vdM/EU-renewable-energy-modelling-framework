...
dis_cap =  hydro2._compute_m3_capacity(
    cap.capacity, cap.weighted_H, eta=config.ror_efficiency)*(3600*dt)# make daily alues
...
for file in dis_files:
    ...
    production = hydro2.compute_hydro_energy_production(
        dis[config.dis_varname]*f, cap.weighted_H).compute()
...
cdo.remapsum(config.t2m_files[0],
...
if config.make_mean_yearly_discharge: 
    cdo.timmean(input= '-yearsum -ensmean %s' % config.mean_discharges_dir+"*.nc",
...
for disf in dis_files:
    ...
    production.to_netcdf(dir_reservoir + os.path.basename(disf).replace(var2[:3], 'reservoir_in'))
...
for cf in capacity_files:
    cdo.remapsum(config.t2m_files[0],
             ...
             readCdf=True,
             options='-f nc')
pv_util_cap = xr.open_dataset(
    dir_bounds+Path(config.pv_util_cap).stem + '_remap.nc')
pv_roof_cap = xr.open_dataset(
    dir_bounds+Path(config.pv_roof_cap).stem + '_remap.nc')
onwind_cap = xr.open_dataset(
    dir_bounds+Path(config.onwind_cap).stem + '_remap.nc')
offwind_cap = xr.open_dataset(
    dir_bounds+Path(config.offwind_cap).stem + '_remap.nc')
...
for r in config.runs:
    ...
    prod = attributes.set_global_attributes(
        prod, 
        f'climate data from run {r}, capacities_eu.nc')
