...
def optimize_reservoirs_usage(
    ...
    maxstep = 3*(meaninflow/meandemand)*(df[variables[0]].diff().abs().max())
...
#     if init != None:
#         model.Eout = pyo.Var(model.T, bounds=(0, model.CapHydro), initialize=init)
#     else:
...
#         elif t == 30:
#             if bound_fraction != None:
#                 return model.Res[t] == model.ResMax * bound_fraction
#             else:
#                  return model.Res[t] == (
...
#                 model.Res[t - 1] + model.Ein[t - 1] - model.Eout[t - 1]
    ...
    def final_storage_state(model, t):
        ...
        if t == model.T.last():
            return model.Res[t] >= (model.ResMax * end_fraction)
        elif t==overlap:
            if overlap > 0:
                return model.Res[t] == model.ResMax * bound_fraction
            else:
...
#         "Amount of energy stored in reservoir at the end of timeseries"
#         # set as the same as the begnning of timeseries
# #         return model.Eout[0] == 0
#         if t  == 0:#model.T.first():
#             return model.Eout[t] == 0 #startflow#init[t+21]
#         else:
#             return pyo.Constraint.Skip
#     model.smoothTrans = pyo.Constraint(model.T, rule=prevRun)
    ...
    # Constrain deveation from demand
    ...
    # # constrain max step
    # def max_step_pos(model,t):
    #     if t == model.T.first():
    #         return pyo.Constraint.Skip
    #     else:
    #         return maxstep >= (model.Eout[t]-model.Eout[t-1])
    # def max_step_neg(model,t):
    #     if t == model.T.first():
    #         return pyo.Constraint.Skip
    #     else:
    #         return maxstep >= -(model.Eout[t]-model.Eout[t-1])
        
    # model.maxsteppos = pyo.Constraint(model.T, rule=max_step_pos)
    # model.maxstepneg = pyo.Constraint(model.T, rule=max_step_neg)
    ...
    return model, df_results, results
