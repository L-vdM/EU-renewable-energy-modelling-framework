...
def _day_length(data, shift_doy=config.shift_doy):
    ...
    P = np.arcsin(
        0.39795 * np.cos(
            0.2163108 + 2 * np.arctan(
                0.9671396 * np.tan(0.00860*(day_of_year-shift_doy))
            )
        )
    )
...
def compute_solar_energy_potential(
    ...
    performance_ratio = _solar_performance_ratio(
        cell_temperature, gamma, ref_temp)
