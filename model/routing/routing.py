...
def pcr_2D_to_netcdf(map, mappcr, varname, attributes=None, no_negative=True, unit='[-]'):
    """
    ...
    lons = np.arange(
        xmin + 0.5*cell_size,
        xmin + (cell_size*nr_cols),
        cell_size
    )
    lats = np.arange(
        ymax - (0.5*cell_size),
        ymax - (cell_size*nr_rows),
        -cell_size
    )
    ...
    ds[varname] = ds[varname].assign_attrs(dict(
...
def accumulate_runoff(inputFile, ds_ldd, ds_routing, outputFile, varname='mrros'):
    ...
    for j in range(0,70):
        ...
        ds0 = xr.concat([_shift_dataset(ds, layer.ldd, i) for i in range(1,10)], 'dir').sum(dim='dir')
