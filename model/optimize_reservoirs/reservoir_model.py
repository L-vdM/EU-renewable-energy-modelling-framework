...
def optimize_reservoirs_usage(df, cap_hydro, cap_conv = None, stor_weeks = None, variables=['netto_demand', 'inflow'], 
                              begin_fraction = 0.0, ts = 24, cyclus = None, 
                              cost_hydro=1, cost_conv = 10):
    """
    ...
    model.CostHydro=  pyo.Param(initialize=cost_hydro,
                           doc='cost of hydropower')
    model.CostConv =  pyo.Param(initialize=cost_conv,
                           doc='cost of fossil fuels')
    model.CapHydro = pyo.Param(initialize=cap_hydro * ts,
                           doc='Max rate of hydropower flow (MWh/day) out')
    if cap_conv is not None:
        model.CapConv = pyo.Param(initialize=cap_conv * ts,
                               doc='Max rate of hydropower flow (MWh/day) out')
    if stor_weeks is None:
        ...
        cap_reservoir = df[[variables[1]]].resample('1Y').sum().mean()[0]/(52/stor_weeks)
        
    model.ResMax = pyo.Param(
        initialize=cap_reservoir, doc='Max storage (MWh)')
    ...
    def storage_state(model, t):
        ...
        if t == model.T.first():
            ...
            return (model.Res[t] == (model.Res[t-1]
                                + model.Ein[t-1] 
                                - model.Eout[t-1] ))
...
#             return pyo.Constraint.Skip
    ...
    def diff_deviation1(model, t):
        return model.diff_dev[t] >= (model.demand_deviation[t]-model.flow_deviation[t])
...
#     cost = sum((model.CostConv * model.Conv[t] + model.CostHydro * model.Eout[t] + model.diff_dev[t]*meaninflow) for t in model.T) 
    ...
    cost = (sum(((model.CostConv * model.Conv[t] + model.CostHydro * model.Eout[t]) + model.diff_dev[t]) for t in model.T)) +  model.ConvMax
        ### set cost function
